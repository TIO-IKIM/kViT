...
CONFIG_PATH="src/configs/train_kspace_2D.yaml"
OUTPUT_BASE="src/output"
MODEL_NAME="train_0.0001_best_prostaterun_kvit"
...
kspace: true       # kspace and hybrid are ignored for MIL mode
hybrid: false       # kspace and hybrid are ignored for MIL mode

# Model configuration
model: KSpaceTransformer       # Model to be used. Options: "vit", "efficientnet", or "resnet" (Image) or "KSpaceTransformer" (k-space)
embed_dim: 256
num_heads: 16
num_layers: 6
ff_dim: 768
...
comment: best_prostaterun_kvit  # Freeform to give additional context to saving file.
EOL

# Train the model
python src/train_kspace.py --config "$CONFIG_PATH"
