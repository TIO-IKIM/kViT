...
class Grappa:
    ...
    def apply_weights(
        self, kspace: np.ndarray, weights: Dict[int, np.ndarray]
    ) -> np.ndarray:
        """
        ...
        with NTF() as frecon:
            ...
            for ii in self.kernel_var_dict["patch_indices"]:
                for xx, yy in zip(
                    self.kernel_var_dict["holes_x"][ii],
                    self.kernel_var_dict["holes_y"][ii],
                ):
                    # Collect sources for this hole and apply weights
                    S = kspace[
                        xx - kx2 : xx + kx2 + adjx, yy - ky2 : yy + ky2 + adjy, :
                    ]
                    S = S[self.kernel_var_dict["patches"][ii, ...]]
                    recon[xx, yy, :] = (weights[ii] @ S[:, None]).squeeze()

            return np.moveaxis(
                (recon[:] + kspace)[kx2:-kx2, ky2:-ky2, :], -1, self.coil_axis
            )
