...
def adc(
    raw_images: np.ndarray, adc_scale: float, b_values: list
...
def b1500(
    adc_map: np.ndarray, b0_img: np.ndarray, adc_scale: float, b_values: List[int]
) -> np.ndarray:
    """
    ...
    b0_intensity = b0_img[
        (adc_map < noise_threshold_max_adc) & (b0_img > noise_threshold_min_b0)
    ]
    ...
    adc_offset = np.where(
        ...
        noise_estimation_adc_offset
        * np.sqrt(np.maximum(1 - ((b0_img / noise_level) ** 2), 0)),
        0,
    )
    ...
    tmp_exponent = (neg_calc_b_value - neg_max_b_value) * np.maximum(
        adc_map, adc_offset
    ) + neg_max_b_value * adc_map
...
def compute_trace_adc_b1500(img_dict: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
    ...
    adc_map, b0_img = map(
        ...
        zip(
            *[
                adc(adc_vol[sl, ...], adc_scale, b_values)
                for sl in range(recon_shape[0])
            ]
        ),
    )
